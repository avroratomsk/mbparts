...
$(document).ready(function () {
  var count = 0;
  var n = $(".reviews__item").length;
  var cell = Math.floor(n / 2);

  if (n % 2 === 0) {
    $(".reviews__item:last-child").clone().appendTo('.reviews__grid');
  }
  ...
  $(".reviews__item").each(function (index) {
    ...
    if (count <= cell) {
      $(this).addClass('reviews__item--prev');
    } else if (count > cell + 1) {
      $(this).addClass('reviews__item--next');
    } else if (count === cell + 1) {
      $(this).addClass('reviews__item--active');
    }
  });

  $(document).on('click', '.reviews__item--next', function () {
    ...
    $('.reviews__item').removeClass('reviews__item--prev reviews__item--next');
    $(".reviews__item:first-child").remove();
