...
gulp.task("global_mb_views", () => {
    return gulp.src(paths.global_mb_views.src)
        .pipe(include({
            prefix: "@@",
            basepath: "@file"
        }))
        .pipe(gulpif(production, replace(".css", ".min.css")))
        .pipe(gulpif(production, replace(".js", ".min.js")))
        .pipe(gulp.dest(paths.global_mb_views.dist))
        .pipe(browsersync.stream());
});
...
gulp.task("mbviews", () => {
    return gulp.src(paths.mbviews.src)
        .pipe(include({
            prefix: "@@",
            basepath: "@file"
        }))
        .pipe(gulpif(production, replace(".css", ".min.css")))
        .pipe(gulpif(production, replace(".js", ".min.js")))
        .pipe(gulp.dest(paths.mbviews.dist))
        .pipe(browsersync.stream());
});
